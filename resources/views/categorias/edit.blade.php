        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('categorias.update', $categoria->id) }}" method="POST">
                    ...
                    @php
                        $tipoSeleccionado = old('tipo_categoria', $categoria->tipo_categoria ?? '');
                    @endphp
                    <div class="mb-3">
                        ...
                        <select name="tipo_categoria" id="tipo_categoria" class="form-select" required>
                            <option value="ingreso" {{ $tipoSeleccionado === 'ingreso' ? 'selected' : '' }}>Ingreso
                            </option>
                            <option value="gasto" {{ $tipoSeleccionado === 'gasto' ? 'selected' : '' }}>Gasto</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="icono" class="form-label">Icono</label>
                        <input type="text" name="icono" id="icono" class="form-control" value="{{ old('nombre', $categoria->icono) }}"
                            placeholder="Ejemplo: bi bi-wallet2">
                    </div>
                    <div class="mb-3">
                        <label for="color" class="form-label">Color</label>
                        <input type="color" name="color" id="color" class="form-control" value="{{ old('color', $categoria->color) }}">
                    </div>
                    <div class="mb-3">
                        <div class="mb-3">
